...
class ModelRunner:
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        _t0 = debug_start("load_model", prefix="tensor")
        load_model(self.model, config.model)
        debug_end("load_model", _t0, prefix="tensor")
    ...
    def _allocate_sample_buffers(self):
        ...
        _t0 = debug_start("_allocate_sample_buffers", prefix="tensor")
        ...
        debug_end("_allocate_sample_buffers", _t0, prefix="tensor")
    ...
    def warmup_model(self):
        _t0 = debug_start("warmup_model", prefix="tensor")
        ...
        self.run(seqs, True)
        torch.cuda.empty_cache()
        debug_end("warmup_model", _t0, prefix="tensor")

    def allocate_kv_cache(self):
        _t0 = debug_start("allocate_kv_cache", prefix="tensor")
        config = self.config
        hf_config = config.hf_config
        ...
        for module in self.model.modules():
            if hasattr(module, "k_cache") and hasattr(module, "v_cache"):
                ...
                layer_id += 1
        debug_end("allocate_kv_cache", _t0, prefix="tensor")

    def prepare_block_tables(self, seqs: list[Sequence]):
        _t0 = debug_start("prepare_block_tables", prefix="tensor")
        max_len = max(len(seq.block_table) for seq in seqs)
        block_tables = [seq.block_table + [-1] * (max_len - len(seq.block_table)) for seq in seqs]
        block_tables = torch.tensor(block_tables, dtype=torch.int32, pin_memory=True).cuda(non_blocking=True)
        debug_end("prepare_block_tables", _t0, prefix="tensor")
        return block_tables

    def prepare_prefill(self, seqs: list[Sequence]):
        _t0 = debug_start("prepare_prefill", prefix="tensor")
        input_ids = []
        positions = []
        cu_seqlens_q = [0]
        cu_seqlens_k = [0]
        ...
        debug_end("prepare_prefill", _t0, prefix="tensor")
        return input_ids, positions

    def prepare_decode(self, seqs: list[Sequence]):
        """Optimized decode preparation using pre-allocated buffers."""
        _t0 = debug_start("prepare_decode", prefix="tensor")
        bs = len(seqs)
        ...
        debug_end("prepare_decode", _t0, prefix="tensor")
        return input_ids, positions
    ...
    def prepare_sample(self, seqs: list[Sequence], is_cfg_batch: bool = False):
        ...
        _t0 = debug_start("prepare_sample", prefix="tensor")
        if is_cfg_batch:
            num_seqs = len(seqs) // 2
    ...
    def run_model(self, input_ids: torch.Tensor, positions: torch.Tensor, is_prefill: bool):
        ...
        if is_prefill or self.enforce_eager or input_ids.size(0) > 512:
            ...
            debug_end("run_model", _t0, prefix="tensor")
            return out
        else:
            bs = input_ids.size(0)
            context = get_context()
    ...
    def capture_cudagraph(self):
        _t0 = debug_start("capture_cudagraph", prefix="tensor")
        config = self.config
        hf_config = config.hf_config
        ...
        debug_end("capture_cudagraph", _t0, prefix="tensor")
